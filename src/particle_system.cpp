...
bool ParticleSystem::CanInsert(std::size_t count) const {
    ...
    const std::size_t activeParticleCount = positions_.size() - std::min(deadParticleCount_, positions_.size());
    return (maxParticles_ - activeParticleCount) >= count;
}
...
bool ParticleSystem::AddParticle(
    ...
    if (positions_.size() < maxParticles_) {
        ...
        ids_.push_back(nextParticleId_++);
        species_.push_back(type);
        return true;
    }

    if (deadParticleCount_ == 0) {
        return false;
    }
    ...
    for (std::size_t i = 0; i < species_.size(); ++i) {
        ...
        masses_kg_[i] = mass_kg;
        charges_C_[i] = charge_C;
        weights_[i] = weight;
        chargeToMass_[i] = charge_C / mass_kg;
        ids_[i] = nextParticleId_++;
        species_[i] = type;
        --deadParticleCount_;
        return true;
    }
...
void ParticleSystem::MarkDead(std::size_t index) {
    if (species_[index] != ParticleType::Dead) {
        ++deadParticleCount_;
    }
    species_[index] = ParticleType::Dead;
}

void ParticleSystem::RestoreSpecies(std::size_t index, ParticleType type) {
    if (species_[index] == ParticleType::Dead && type != ParticleType::Dead && deadParticleCount_ > 0) {
        --deadParticleCount_;
    }
    species_[index] = type;
}
...
bool ParticleSystem::IsFiniteState() const {
    ...
    for (std::size_t i = 0; i < positions_.size(); ++i) {
        ...
        if (!std::isfinite(masses_kg_[i]) || !std::isfinite(charges_C_[i]) || !std::isfinite(weights_[i]) ||
            !std::isfinite(chargeToMass_[i]) || weights_[i] <= 0.0) {
            return false;
        }
        if (ids_[i] == 0) {
            return false;
